...
from scripts.Utils import load_tilemap_imges
        ...
        while True:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    if self.shift:
                        if event.button == 4:
                            self.tile_variant= (self.tile_variant -1) % len(self.assets[self.tile_list[self.tile_group]]) # mit "%" können wir wie ein loop machen 
                        if event.button == 5:
                            self.tile_variant= (self.tile_variant +1) % len(self.assets[self.tile_list[self.tile_group]]) # mit "%" können wir wie ein loop machen 
                    else:
                        if event.button == 4:
                            self.tile_group= (self.tile_group -1) % len(self.tile_list) # mit "%" können wir wie ein loop machen 
                            self.tile_variant = 0
                        if event.button == 5:
                            self.tile_group= (self.tile_group +1) % len(self.tile_list) # mit "%" können wir wie ein loop machen 
                            self.tile_variant = 0
